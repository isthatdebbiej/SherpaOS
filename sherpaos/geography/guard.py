...
class GeographicRiskGuard:
    ...
    def _evaluate(self, mission_context: MissionContext | None, now: float) -> GuardReport:
        ...
        provenance = {
            ...
            "elevation_m": f"{float(mc.elevation_m):.0f}",
            "route_slope_deg": "unavailable" if slope_deg is None else f"{float(slope_deg):.2f}",
            "distance_to_safe_waypoint_m": f"{float(distance_m):.0f}",
