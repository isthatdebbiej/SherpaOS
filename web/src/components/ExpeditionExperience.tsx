...
function JournalDrawer({ day, onClose, onStep }: { day: ExpeditionDay; onClose: () => void; onStep: (delta: -1 | 1) => void }) {
  ...
  return <motion.aside className={`journalDrawer ${side}`} role="dialog" aria-modal="false" aria-labelledby="journal-title" initial={{ x: side === "left" ? "-102%" : "102%", opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: side === "left" ? "-102%" : "102%", opacity: 0 }} transition={{ type: "spring", stiffness: 250, damping: 28 }}>
    <header className="journalHeader"><div><span>FIELD REFLECTION · DAY 0{day.day}</span><h2 id="journal-title">{day.camp}</h2><p>{day.date} · {day.reflectionTimestamp} {day.timezone}</p></div><button onClick={onClose} aria-label="Close journal">×</button></header>
    <div className="journalFacts"><span><small>ALTITUDE</small>{day.altitude.toLocaleString()} m</span><span><small>WEATHER</small>{day.weather}</span><span><small>MOOD</small>{day.moodCaption}</span></div>
    <section className="summaryCard"><span className="sectionLabel">DAILY SUMMARY <i>VERIFIED SUMMARY</i></span><p>{day.diary.summary}</p></section>
    <section><span className="sectionLabel">EPISODE TIMELINE</span><div className="eventTimeline">{day.events.map(item => <article key={item.id}><time>{item.time}</time><div><h3>{item.title}</h3><p>{item.description}</p><code>{item.evidence}</code></div></article>)}</div></section>
    <div className="reflectionGrid"><section><span>CHALLENGE</span><p>{day.diary.challenge}</p></section><section><span>RECOVERY</span><p>{day.diary.recovery}</p></section><section><span>PROUD MOMENT</span><p>{day.diary.proudMoment}</p></section><section><span>LESSON</span><p>{day.diary.lesson}</p></section></div>
    <blockquote>“{day.diary.tomorrowIntent}”<small>NEXT INTENTION · EXPRESSIVE REFLECTION</small></blockquote>
    <footer className="journalNav"><button disabled={day.day === 1} onClick={() => onStep(-1)}>← Previous day</button><span>{day.day} / 5</span><button disabled={day.day === 5} onClick={() => onStep(1)}>Next day →</button></footer>
  </motion.aside>;
}
...
export function ExpeditionExperience() {
  ...
  return <main className={`fieldApp ${active}View ${drawerOpen ? "drawerVisible" : ""}`}>
    ...
    <footer className="appFooter"><span>EVEREST · 27.9881° N</span><span>REFLECTIONS ARE EXPRESSIVE · EVIDENCE IS VERIFIED</span></footer>
  </main>;
}
