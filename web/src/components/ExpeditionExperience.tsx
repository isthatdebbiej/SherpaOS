...
function JournalDrawer({ day, onClose, onStep }: { day: ExpeditionDay; onClose: () => void; onStep: (delta: -1 | 1) => void }) {
  ...
  return <motion.aside className={`journalDrawer ${side}`} role="dialog" aria-modal="false" aria-labelledby="journal-title" initial={{ x: side === "left" ? "-102%" : "102%", opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: side === "left" ? "-102%" : "102%", opacity: 0 }} transition={{ type: "spring", stiffness: 250, damping: 28 }}>
    ...
    <section className="operatorNarrative"><span className="sectionLabel">OPERATOR ASSESSMENT · VERIFIED SIMULATION EVIDENCE</span><h3>Evidence scope</h3><p>{day.diary.summary}</p><h3>Why the scenario was risky or safe</h3><p>{day.diary.challenge}</p><p>{day.diary.recovery}</p><h3>Interpretation</h3><p>{day.diary.lesson}</p><h3>Real-world telemetry status</h3><p>{day.diary.tomorrowIntent}</p></section>
