...
export function LiveSafetyDemo() {
  ...
  return <section className="liveDemo" aria-label="Live SherpaOS safety supervisor">
    <header><div><span>VULTR · LIVE SUPERVISOR</span><h1>Robot safety decision</h1></div><b className={connected ? "connected" : "offline"}>{connected ? "STREAM CONNECTED" : "RUNTIME OFFLINE"}</b></header>
    <div className="liveGrid">
      <article className="robotFeed"><div className="feedLabel">QUALIFIED G1 SIMULATION REPLAY</div><video src={`${API}/stream/robot.mp4`} autoPlay loop muted playsInline controls preload="auto" aria-label="Qualified MuJoCo G1 simulation replay"/><div className="feedEmpty">Waiting for Vultr robot video</div></article>
