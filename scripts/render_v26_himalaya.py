...
from sherpaos.sim.v26_playground import DEFAULT_POSE, JOINTS, KD, KP, POLICY_SHA256
from sherpaos.sim.v26_playground import V26ObservationHistory, action_target
from sherpaos.sim.v26_playground import projected_gravity, robot_visibility


def arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--policy", type=Path, default=Path("var/policies/v26/g1_v26_iter42290.onnx"))
    parser.add_argument("--g1-dir", type=Path, default=Path("third_party/mujoco_menagerie/unitree_g1"))
    parser.add_argument("--output", type=Path, default=Path("artifacts/playground/v26-himalaya-visible.mp4"))
...
def main() -> None:
    ...
    if not args.policy.is_file() or hashlib.sha256(args.policy.read_bytes()).hexdigest() != POLICY_SHA256:
    ...
    qadr = np.array([model.jnt_qposadr[mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, name)] for name in JOINTS])
    aids = np.array([mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_ACTUATOR, name) for name in JOINTS])
    ...
    encoder = subprocess.Popen(["ffmpeg", "-loglevel", "error", "-y", "-f", "rawvideo", "-pix_fmt", "rgb24", "-s", "1280x720", "-r", "25", "-i", "-", "-an", "-vcodec", "libx264", "-crf", "20", "-pix_fmt", "yuv420p", str(args.output)], stdin=subprocess.PIPE)
    renderer = mujoco.Renderer(model, height=720, width=1280)
    try:
        for step in range(args.steps):
            velocity = np.zeros(6)
            mujoco.mj_objectVelocity(model, data, mujoco.mjtObj.mjOBJ_BODY, pelvis, velocity, 1)
            observation = history.build(data.qpos[qadr], data.qpos[3:7], velocity[:3], np.array([args.command_vx, 0, 0, 0], np.float32))
            ...
            min_height, max_tilt, completed = min(min_height, float(data.qpos[2])), max(max_tilt, tilt), step + 1
            if step % 2 == 0:
                camera = mujoco.MjvCamera()
                camera.type, camera.distance, camera.azimuth, camera.elevation = mujoco.mjtCamera.mjCAMERA_FREE, 4.0, 90, -20
    ...
    if min_pixels < 5000 or min_height_px < 120:
        ...
        raise SystemExit(f"robot visibility gate failed: {min_pixels} pixels, {min_height_px}px tall")
    report = {"policy": "v26 iter42290 frozen", "controller_only": True, "sherpaos_intervention": False, "steps": completed, "fell": fell, "forward_m": float(data.qpos[0] - start_x), "min_height_m": min_height, "max_tilt_deg": max_tilt, "minimum_robot_pixels": min_pixels, "minimum_robot_height_px": min_height_px, "visibility_gate": "PASS"}
